-- ================================================================
-- DATABASE SCHEMA SYNC & RELATIONSHIP FIX (FINAL STABLE)
-- ================================================================
-- Memperbaiki error "cannot use column reference in DEFAULT"
-- Menggunakan EXECUTE untuk setting default secara dinamis.
-- ================================================================
...
BEGIN
    ...
    IF v_current_type = 'text' THEN
        ...
        WHERE theme_id IS NULL 
           OR theme_id = '' 
           OR theme_id NOT SIMILAR TO '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'; -- Simplified check or just anything not 36 chars
    ...
    IF v_current_type = 'text' THEN
        ...
        WHERE assigned_theme_id IS NULL 
           ...
           OR assigned_theme_id NOT SIMILAR TO '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}';
